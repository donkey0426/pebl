...
## Make the cIGT single-task web build (production):
## Same as em-opt-real, but also preloads the battery/cigt task directory
## and uses a dedicated shell HTML that auto-launches cigt.pbl with a
## generated subject id, instead of requiring external callMain() wiring.
em-cigt:
